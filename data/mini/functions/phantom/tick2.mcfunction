...
# 清理错位的三叉戟
execute as @a[tag=mini_running] unless score @s countdown_fast matches 1.. unless data entity @s Inventory[{id:"minecraft:trident",Slot:8b,tag:{GameItem:1b}}] if data entity @s Inventory[{id:"minecraft:trident",tag:{GameItem:1b}}] run function mini:phantom/game/give_trident
...
kill @e[type=item,nbt={Item:{id:"minecraft:trident",tag:{GameItem:1b}}}]