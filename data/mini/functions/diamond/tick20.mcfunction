...
# 90s / 60s：雷达提示
execute if score $countdown mem matches 90 run tellraw @a ["",{"text": ">> ","color": "gold","bold": true},"玩家雷达将在 ",{"text": "30 秒 ","color": "gold"}, "后开启!"]
...
execute if score $countdown mem matches 60 run tellraw @a ["",{"text": ">> ","color": "gold","bold": true},"玩家雷达已开启, 游戏将在 ",{"text": "60 秒 ","color": "gold"}, "后结束!"]
execute if score $countdown mem matches 60 as @a run function lib:sounds/music/mini_fast
