...
execute if score $count mem matches ..2 run tellraw @a ["",{"text": ">> ","color":"red","bold": true},"需要至少 3 人准备以开始游戏, ",{"text":"当前仅有 ","color":"red"},{"score": {"name": "$count","objective": "mem"},"color":"red"},{"text": " 人","color":"red"}]
execute if score $count mem matches ..2 run scoreboard players set $countdown mem 0

# 提示未准备
execute if score $countdown mem matches 10 run tellraw @a[tag=!lobby_ready] [{"text":"","color":"red"},{"text":">> ","bold": true},"你现在还没有准备, 可以丢出快捷栏最后一格的物品准备!"]
