...
# 优化下一次游戏的选择：生成一个 1~[最大游戏编号] 的 randomarray，每次从其中取下一个数。如果数组取完了，就生成一个新数组，但是要保证新数组的开头和老数组的结尾不一样，这样就不会连续玩两次相同的游戏了
execute unless data storage lib:randarray out[0] run function ltw:state/3/newarray
execute store result score $mini_type mem run data get storage lib:randarray out[0]
data remove storage lib:randarray out[0]
