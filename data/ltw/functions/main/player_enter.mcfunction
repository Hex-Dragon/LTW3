...
execute if entity @s[team=!watching] unless score @s game_id = $ game_id run function ltw:main/player_enter_watcher
# 如果不是旁观且局号正确：游戏中掉线
execute if entity @s[team=!watching] if score @s game_id = $ game_id run function ltw:main/player_enter_rejoin
...
# FIXME: [HIM/010] 把道具放到 “已锁定” 格子导致道具消失
