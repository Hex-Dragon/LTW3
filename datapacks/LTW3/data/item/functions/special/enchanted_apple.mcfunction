...
execute if score @s mini_heart matches 1.. run tellraw @a ["",{"text": ">> ","color":"aqua","bold": true},{"selector": "@s","color":"aqua"}," 食用了附魔金苹果, 生命加倍！"]

# 无生命计数：恢复生命
execute unless score @s mini_heart matches 1.. run effect give @s instant_health 1 1 true
execute unless score @s mini_heart matches 1.. run tellraw @a ["",{"text": ">> ","color":"aqua","bold": true},{"selector": "@s","color":"aqua"}," 食用了附魔金苹果！"]
